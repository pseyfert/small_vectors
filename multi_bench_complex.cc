...
inline size_t get_loopend(filling PICKED_FILLING) {
  if (EXACTFULL == PICKED_FILLING)
    return CONTAINERSIZE;
  if (UNDERFULL == PICKED_FILLING)
    return CONTAINERSIZE - random_gen() % FILLING_OFFSET;
  if (OVERFULL == PICKED_FILLING)
    return CONTAINERSIZE + random_gen() % FILLING_OFFSET;
  return 0;
}

template <typename CONTAINER, bool RESERVE>
inline void reserve_on_compile_demand(
    typename std::enable_if<RESERVE, CONTAINER>::type &asdf, size_t loopend) {
  asdf.reserve(loopend);
}

template <typename CONTAINER, bool RESERVE>
inline void
reserve_on_compile_demand(typename std::enable_if<!RESERVE, CONTAINER>::type &,
                          int) {}
...
using OUTER_CONTAINER =
    OUTER_CONTAINER_TYPE<bigthing<INNER_CONTAINER, RESERVE, PICKED_FILLING>>;
...
template <typename INNER_CONTAINER, bool RESERVE, filling PICKED_FILLING>
using OUTER_CONTAINER_WITH_ALLOCATOR = OUTER_CONTAINER_TYPE<
    bigthing_custom_alloc<INNER_CONTAINER, RESERVE, PICKED_FILLING>>;
...
template <bool RESERVE, filling PICKED_FILLING> struct reservething {
  reservething(size_t N) {
    ...
    size_t loopend = get_loopend(PICKED_FILLING);
    reserve_on_compile_demand<decltype(m_collection), RESERVE>(m_collection,
                                                               loopend);
    for (size_t i = 0; i < loopend; i++) {
      m_collection.push_back(N % (1 + i));
    }
...
using reservething_CONTAINER =
    OUTER_CONTAINER_TYPE<reservething<RESERVE, PICKED_FILLING>>;
